/*
Usage: ./RFSniffer [<pulseLength>] [-v] [-s]
	[] = optional
...
void printHelp()
{
	printf("Usage: ./RFSniffer [<pulseLength>] [-v] [-s] [-h] [-?]\n");
        printf("[] = optional\n");
        printf("\n");
        printf("-h or -?\tShows this help text\n");
        printf("-v\t\tVerbose decoding\n");
        printf("-s\t\tSimplified decoding\n");
}
	...
	while(1) {
		if (mySwitch.available()) {
			...
			if (value == 0) {
				...
				else if ((argc > 1 && strcmp(argv[1],"-s") == 0) || (argc > 2 && strcmp(argv[2],"-s") == 0))
				{
